...
def read_init_file():
    ...
    with open(file_name, 'r') as f:
        tmp = f.readline().split(":")[1].split(",")
        screen_size = [int(tmp[0]),int(tmp[1])]
        max_wind = float(f.readline().split(":")[1])
        show_tail = int(f.readline().split(":")[1])
        targets = []
        for line in f:
            if line.startswith("target"):
                tmp = line.split(":")[1].split(",")
                targets.append([int(tmp[0]),int(tmp[1])])
    return (screen_size, max_wind, show_tail, targets)

screen_size, max_wind, show_tail, targets = read_init_file()
